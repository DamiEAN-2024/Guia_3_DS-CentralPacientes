...
public class InterfazCentralPacientes extends JFrame {
    ...
    public void reasignarPaciente() {
    	 // Verificar si hay pacientes registrados
        if (central.darPacientes().isEmpty()) {
            JOptionPane.showMessageDialog(this, "No hay pacientes registrados para reasignar.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Crear un cuadro de diálogo para seleccionar el paciente
        String[] pacientesNombres = central.darPacientes().stream()
                                        .map(Paciente::darNombre)
                                        .toArray(String[]::new);
        String pacienteSeleccionado = (String) JOptionPane.showInputDialog(this, "Seleccione el paciente a reasignar:",
                "Reasignar paciente", JOptionPane.QUESTION_MESSAGE, null, pacientesNombres, pacientesNombres[0]);

        // Crear un cuadro de diálogo para seleccionar la nueva clínica
        String[] clinicas = central.darListaClinicas().toArray(new String[0]);
        String clinicaSeleccionada = (String) JOptionPane.showInputDialog(this, "Seleccione la nueva clínica:",
                "Reasignar clínica", JOptionPane.QUESTION_MESSAGE, null, clinicas, clinicas[0]);

        if (pacienteSeleccionado != null && clinicaSeleccionada != null) {
            Paciente paciente = central.localizarPorNombre(pacienteSeleccionado);
            if (paciente != null) {
                paciente.cambiarClinica(clinicaSeleccionada);
                JOptionPane.showMessageDialog(this, "El paciente " + pacienteSeleccionado + " fue reasignado a la clínica " + clinicaSeleccionada);
            }
        } 
    }
}